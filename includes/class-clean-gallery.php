...
final class Clean_Gallery {
	...
	public function gallery_shortcode( $output, $attr ) {
		...
		if ( empty( $this->args['include'] ) ) {
			$attachments = get_children( array_merge( array(
	...
	public function get_gallery_item( $attachment ) {
		...
		$output = "\n\t\t\t\t\t<{$this->args['itemtag']} class='gallery-item' itemprop='associatedMedia' itemscope itemtype='{$itemtype}'>";
	 ...
	 *
	 * @access public
	 * @param  string $link The content link.
	 * @return string
	 */
